...
from api_with_cookie import update_cookie, get_cookie
...
def process_from_query_start():
    ...
    if is_success(rsp):
        ...
        if is_success(rsp):
            ...
            # 缓存乘客信息, 不请求12306(延迟高)
            passengers_json = "cache/getPassengerDTOs.json"
            if not os.path.exists(passengers_json):
                log('获取乘客信息列表')
                rsp = get_passengers(token)
                # log(f"get_passengers: '{rsp}'")
                with open(passengers_json, 'w') as f:
                    json.dump(rsp, f, ensure_ascii=False, indent=4)
                log(f'乘客信息列表已缓存至文件"{passengers_json}"')
            else:
                log(f'从缓存文件"{passengers_json}"中读取乘客信息')
            ...
            if passenger_list is not None and len(passenger_list) > 0:
                ...
                if check_is_can_select_seat_or_bed(rsp):
                    ...
                    #rsp = get_queue_count(token, selected_train_info)
                        ...
                        # log(f"encrypted_data = {encrypted_data}")
                    #    log('确定加入排队队列')
...
def start():
    ...
    # 等待到指定时间, 再开始抢票
    if 'ticketRushTime' in config_dict:
        log(f'等待中, 启动抢票时间: {config_dict.get("ticketRushTime")}')
        wait_until_time()
